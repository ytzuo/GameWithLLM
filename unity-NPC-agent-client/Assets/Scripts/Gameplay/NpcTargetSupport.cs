...
public interface IGameplayWorldTarget
{
    string WorldTargetId { get; }
    string WorldTargetDisplayName { get; }
    string WorldTargetCategory { get; }
    bool IsDynamicWorldTarget { get; }
}

public sealed class NpcTargetRecord
{
...
public static class NpcTargetSupport
{
    ...
    public static List<NpcTargetRecord> FindTargets()
    {
        ...
        for (int i = 0; i < behaviours.Length; i++)
        {
            ...
            if (!(behaviour is IGameplayWorldTarget target) ||
                string.IsNullOrWhiteSpace(target.WorldTargetId))
                continue;
            targets.Add(new NpcTargetRecord(
                target.WorldTargetId.Trim(),
                string.IsNullOrWhiteSpace(target.WorldTargetDisplayName)
                    ? behaviour.gameObject.name
                    : target.WorldTargetDisplayName.Trim(),
                target.WorldTargetCategory,
                behaviour.gameObject,
                target.IsDynamicWorldTarget));
        }
        ...
        for (int i = 1; i < targets.Count; i++)
        {
            ...
            {
                throw new GameplayOperationException(
                    "TARGET_ID_AMBIGUOUS",
