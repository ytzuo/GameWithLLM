...
public class NpcEntity : MonoBehaviour, IGameObjectAgentEntity, IGameplayWorldTarget
{
    ...
    public float InventoryInteractionRange => inventoryInteractionRange;
    public bool IsOnNavMesh => _navAgent != null && _navAgent.isOnNavMesh;
    public string WorldTargetId => string.IsNullOrWhiteSpace(npcId) ? string.Empty : $"npc:{npcId.Trim()}";
    ...
    private void OnEnable() => RuntimeAvailabilityChanged?.Invoke(this, true);

    private void OnDisable() => RuntimeAvailabilityChanged?.Invoke(this, false);
    ...
    public ValueTask<AgentToolResult> MoveToTargetAsync(
        string targetId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            BeginMove(targetId, approachDistance);
        }
        catch
        {
            ClearActiveMovement();
    ...
    private void BeginMove(string targetId, float approachDistance)
    {
        if (_navAgent == null)
            throw new GameplayOperationException("NAV_AGENT_MISSING", $"NPC '{npcId}' 没有 NavMeshAgent。");
        if (!_navAgent.isOnNavMesh)
            throw new GameplayOperationException("NPC_NOT_ON_NAVMESH", $"NPC '{npcId}' 当前不在 NavMesh 上。");

        _activeMoveTarget = NpcTargetSupport.ResolveUniqueTarget(targetId);
        if (_activeMoveTarget.GameObject == gameObject)
        {
            _activeMoveTarget = null;
            throw new GameplayOperationException("TARGET_IS_SELF", "NPC 不能移动到自己身边。");
        }

        _activeApproachDistance = approachDistance > 0f ? approachDistance : moveStoppingDistance;
        ...
        {
            _activeMoveTarget = null;
            throw new GameplayOperationException(errorCode, errorMessage);
        }
