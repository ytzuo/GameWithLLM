...
$testParent = Join-Path $workspace '.tmp\a7-promotion-tests'
$testRoot = Join-Path $testParent ([Guid]::NewGuid().ToString('N'))
$repository = Join-Path $testRoot 'repository'
$publish = Join-Path $testRoot 'publish'
$promote = Join-Path $PSScriptRoot 'Promote-A7Release.ps1'

function New-Candidate {
    param([string]$ReleaseId, [string]$PayloadText)
    $candidate = Join-Path $repository ("Artifacts\A7\{0}" -f $ReleaseId)
...
try {
    ...
    Write-Output 'A7 promotion tests passed: publish, interrupted retry, tamper rejection, pointer preservation, v2, rollback.'
}
finally {
    ...
    if (-not $resolvedTarget.StartsWith($resolvedParent, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing to clean unexpected A7 test directory '$resolvedTarget'."
    }
