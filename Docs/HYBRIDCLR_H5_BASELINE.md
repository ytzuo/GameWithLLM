...
- `agent_messages.zh-CN.json` 和 `ui.zh-CN.json` 使用稳定文本键；工具结果的主要
  自然语言消息已通过 `ClientTextCatalogs` 解析，错误码和日志事件名仍在代码中。
...
- H5 release 为 `h5-local-1.0.0`，ToolSet 为 `2.0.0`。Smoke 包因接入稳定
  agent message 文本键而将包版本提升到 `1.1.0`、实现版本提升到 `2.0.0`；
...
- Unity 全项目 EditMode：15/15 通过，结果位于本地
